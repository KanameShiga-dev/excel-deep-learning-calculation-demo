...
第1回で作った `score` を、Sigmoid、tanh、ReLU、Softmaxで変換します。出力範囲や勾配の大きさを表で確認します。
...
5. Excel上の `input_value`, `logit`, `selected_function`, `temperature`, `display_mode` を変えて、返却表がどう変わるかを見る。
...
| `01_INPUT G6:I9` | `selected_function` | `selected_output`, `selected_gradient` が変わる |
| `01_INPUT G6:I9` | `temperature` | Softmax分布の尖り方が変わる |
| `01_INPUT G6:I9` | `display_mode` | 返却されるDataFrameが変わる |
